      ...
      <div class="page-title">
          <div class="title_left">
            <h3>Semua Data User
              ...
              <small>
                <a href="{{route('user.create')}}" class="btn btn-primary btn-xs"><i class="fa fa-plus"></i> Tambah data </a>
              </small>
              @endif
            </h3>
          </div>
                            ...
                            <td>
                              ...
                              @if ($item->role_id == 1)
                                @if (Auth::user()->role_id == 1)
                                  ...
                                  <a href="{{route('user.edit',$item->id)}}" class="btn btn-info btn-xs"><i class="fa fa-pencil"></i> Edit </a>                                    
                                @endif
                              @else 
                                <a href="{{route('user.edit',$item->id)}}" class="btn btn-info btn-xs"><i class="fa fa-pencil"></i> Edit </a>
                              @endif
                              @if ($item->role_id != 1)
                                <a data-href="{{route('user.destroy',$item->id)}}" data-toggle="modal" data-target="#confirm-delete" class="btn btn-danger btn-xs"><i class="fa fa-trash"></i> Hapus </a>  
                                {{-- <a data-href="{{route('user.change-status',$item->id)}}" data-status="{{$item->active}}" data-toggle="modal" data-target="#confirm-user" class="btn btn-{{$item->active ? 'danger' : 'success'}} btn-xs"><i class="fa fa-{{$item->active ? 'close' : 'check'}}"></i> {{$item->active ? 'Nonaktif' : 'Aktifkan'}} </a> --}}
                              @endif
                            </td>
                        </tr>
                      @endforeach
