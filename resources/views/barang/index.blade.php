    ...
    <div class="">
      <div class="page-title">
          <div class="title_left">
            <h3>Semua Data Barang
              @if (Auth::user()->role_id == 1)
                  
              <small>
                <a href="{{route('barang.create')}}" class="btn btn-primary btn-xs"><i class="fa fa-plus"></i> Tambah data </a>
              </small>
              @endif
            </h3>
          </div>
                    ...
                    <tbody>
                      ...
                      @foreach ($data as $key=>$item)
                        ...
                        <tr>
                            ...
                            <td>{{$item->stock}}</td>
                            <td>
                              <img src="{{asset($item->gambar)}}" alt="" style="max-width:100px;">
                            </td>
                            @if (Auth::user()->role_id == 1)
                            <td>                                  
                                <a href="{{route('barang.edit',$item->id)}}" class="btn btn-info btn-xs"><i class="fa fa-pencil"></i> Edit </a>                                    
                                <a data-href="{{route('barang.destroy',$item->id)}}" data-toggle="modal" data-target="#confirm-delete" class="btn btn-danger btn-xs"><i class="fa fa-trash"></i> Hapus </a>  
                            </td>
                            @endif
                        </tr>
                      @endforeach
